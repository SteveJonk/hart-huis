...
> Auto-maintained by OpenWolf. Last scanned: 2026-08-26T16:07:36.352Z
...
- `check-realworks.ts` — Kleinste ding dat faalt als de Realworks-mapping breekt. (~2391 tok)
  - fn `waarde` L82-103 (~380 tok)
  - fn `metFotos` L104-115 (~86 tok)
  - fn `inSanity` L116-169 (~614 tok)
  - fn `checkVerouderdQuery` L170-213 (~395 tok)
- `check-rich-text.ts` — Smallest thing that fails if de omzetting naar Portable Text breekt (`npm run check:richtext`): stijlen, lijsten, links, **vet** en unieke _key's. (~900 tok)
...
- `wipe-objecten.ts` — Gooit álle `woning`-documenten weg plus de foto's en brochures die eraan (~828 tok)
  - fn `main` L27-89 (~571 tok)
...
- `privacy.ts` — Seeds /privacyverklaring (pageOpener + richText + ctaBand). Zet de tekst uit `privacy-content.ts` om met `toPortableText()`. (~450 tok)
  - fn `buildPrivacyContent` L20-52 (~380 tok)
  - fn `seedPrivacy` L54-61 (~90 tok)
...
- `haarlem.ts` — Seeds de Haarlem-FAQs en /haarlem (stad-template: pageHero, factBar, mediaText, iconCards, regionBlock ×2, quoteBand, faqs, ctaBand — alleen bestaande blocks). (~1150 tok)
  - fn `buildHaarlemContent` L20-133 (~1000 tok)
  - fn `seedHaarlem` L135-144 (~90 tok)
...
- `route.ts` — Haalt de actieve objecten op bij Realworks en zet ze als `woning`-documenten (~4014 tok)
  - class `FeedError` L50-51 (~10 tok)
  - fn `fetchFeed` L52-88 (~348 tok)
  - fn `existingAssets` L89-101 (~142 tok)
  - fn `uploadFromUrl` L102-114 (~134 tok)
  - fn `importObjects` L115-255 (~1419 tok)
  - fn `verouderdeObjecten` L256-267 (~125 tok)
  - fn `depubliceer` L268-285 (~162 tok)
  - fn `ruimOp` L286-297 (~94 tok)
  - fn `handle` L298-401 (~1006 tok)
  - fn `GET` L402-405 (~22 tok)
  - fn `POST` L406-409 (~22 tok)
  - fn `OPTIONS` L410-413 (~38 tok)
...
- `CompareCards.tsx` — Ruimte boven de sectie, nodig als er een gekleurde band vóór staat. (~1799 tok)
  - fn `IconCheck` L28-35 (~62 tok)
  - fn `IconCross` L36-44 (~80 tok)
  - fn `CompareCards` L45-168 (~1441 tok)
...
- `ContactWays.tsx` — Four ways to get in touch, pulled up over the hero. (~694 tok)
  - fn `ContactWays` L16-66 (~566 tok)
- `CrossLinks.tsx` — DEFAULTS (~641 tok)
  - fn `CrossLinks` L21-63 (~506 tok)
...
- `IconCards.tsx` — Drie iconkaarten, geen foto. (~694 tok)
  - fn `IconCards` L27-68 (~487 tok)
- `Intro.tsx` — DEFAULTS (~1518 tok)
  - fn `renderHighlightedTitle` L56-79 (~136 tok)
  - fn `Intro` L80-174 (~921 tok)
- `Listings.tsx` — Pill tone: available (white), sold subject to conditions (sand), sold (burgundy). (~2011 tok)
  - fn `IconArea` L97-108 (~74 tok)
  - fn `IconRooms` L109-125 (~127 tok)
  - fn `toListing` L126-155 (~224 tok)
  - fn `ListingCard` L156-201 (~478 tok)
...
- `Reviews.tsx` — Deelcijfertabel per review. Uit op de homepage, aan op /beoordelingen. (~3090 tok)
  - fn `ReviewMeta` L50-78 (~318 tok)
  - fn `ReviewGrades` L79-105 (~277 tok)
  - fn `ReviewCard` L106-195 (~918 tok)
  - fn `Reviews` L196-306 (~1180 tok)
- `RichText.tsx` — Lopende tekst uit Portable Text (koppen, lijsten, links in een alinea). Het enige blok met Portable Text; voor documentpagina's als de privacyverklaring. (~1000 tok)
  - fn `RichText` L22-38 (~230 tok)
...
- `Steps.tsx` — DEFAULTS (~1728 tok)
  - fn `stepImageSrc` L40-43 (~34 tok)
  - fn `Steps` L44-165 (~1406 tok)
...
- `ValueCards.tsx` — DEFAULTS (~932 tok)
  - fn `ValueIcon` L24-60 (~273 tok)
  - fn `ValueCards` L61-105 (~500 tok)
...
- `FormRenderer.tsx` — Public half of the reCAPTCHA settings — the secret stays server-side. (~3102 tok)
...
- `ObjectContactDialog.tsx` — Text on the button that opens the dialog. (~1166 tok)
  - fn `IconClose` L20-35 (~166 tok)
  - fn `ObjectContactDialog` L36-129 (~829 tok)
...
- `ObjectGallery.tsx` — Client. Three-photo header grid + "Alle foto's" lightbox (arrow keys, Escape, focus return). Lightbox shows a spinner while the active photo loads and preloads its two neighbours. Carries `data-solid-header`. (~2500 tok)
  - fn `IconBack` L18-25 (~62 tok)
  - fn `IconPhotos` L26-38 (~192 tok)
  - fn `Spinner` L40-60 (~230 tok)
  - fn `ObjectGallery` L69-294 (~2100 tok)
...
- `Reveal.tsx` — delayClass (~259 tok)
- `RevealLink.tsx` — delayClass (~284 tok)
...
- `haarlem-content.ts` — Copy voor /haarlem (stad-template): HAARLEM_HERO, HAARLEM_FACTS(+_NOTE), HAARLEM_INTRO, HAARLEM_DIENSTEN, HAARLEM_WIJKEN, HAARLEM_QUOTE, HAARLEM_FAQ, HAARLEM_OMGEVING, HAARLEM_CTA, HAARLEM_SEO. Kopie-basis voor een volgende stad. (~2400 tok)
...
- `realworks.ts` — Vertaalt een object uit de Realworks-feed (`/wonen/v3/objecten`) naar een (~4868 tok)
  - fn `label` L98-105 (~104 tok)
  - fn `sentence` L106-116 (~112 tok)
  - fn `plaatsnaam` L117-126 (~76 tok)
  - fn `slugify` L127-146 (~156 tok)
  - fn `euro` L147-147 (~21 tok)
  - fn `datum` L148-155 (~56 tok)
  - fn `rij` L156-188 (~359 tok)
  - fn `fotoUrl` L189-197 (~110 tok)
  - fn `mediaFilename` L198-207 (~95 tok)
  - fn `toWoning` L208-426 (~2061 tok)
  - fn `zonderBestandsnaam` L427-451 (~232 tok)
  - fn `planMedia` L452-476 (~242 tok)
  - fn `vrijeKey` L477-495 (~195 tok)
  - fn `verouderingsGrens` L496-510 (~151 tok)
...
- `privacy-content.ts` — Concepttekst van de privacyverklaring, in de schrijfvorm van `rich-text.ts`. Plekken tussen [rechte haken] moeten nog ingevuld. (~3000 tok)
- `rich-text.ts` — Schrijfvorm voor het richText-blok ([label](href), **vet**) plus `toPortableText()`. Puur, getest door check:richtext. (~800 tok)
  - fn `withStrong` L47-69 (~230 tok)
  - fn `toSpans` L76-97 (~240 tok)
  - fn `toPortableText` L100-125 (~250 tok)
...
- `richTextType.ts` — Lopende tekst met koppen, lijsten en links (Portable Text). Voor documentpagina's: privacy, voorwaarden, disclaimer. (~520 tok)
...
- `RealworksTool.tsx` — Knop in de studio die de Realworks-import op de site aanroept. De route doet (~1266 tok)
  ...
  - fn `RealworksObjecten` L54-119 (~716 tok)
