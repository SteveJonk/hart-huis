...
- `check-jsonld.ts` — Smallest thing that fails if de structured data stilletjes verandert (`npm run check:jsonld`). (~2760 tok)
  - fn `node` L38-47 (~90 tok)
  - fn `checkStatsQuery` L262-279 (~200 tok)
...
- `json-ld.ts` — Structured data (schema.org) opgebouwd uit Sanity: organisatie, site, WebPage/FAQPage, kruimelpad, woning + Offer. Puur, geen fetch. (~4010 tok)
  ...
  - fn `webPageJsonLd` L239-261 (~380 tok)
  ...
  - fn `residenceJsonLd` L365-401 (~600 tok)
  - fn `offerJsonLd` L403-423 (~230 tok)
