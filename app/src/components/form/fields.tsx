...
/** `stacked` is the contact page's roomy form, `compact` the hero card's. */
export type FormFieldVariant = 'stacked' | 'compact';

const controlBase = cn(
  'w-full rounded-[3px] border border-ink/16 bg-white text-ink',
...
const VARIANTS = {
  stacked: {
    wrapper: 'mb-5',
    label: 'mb-[9px] block text-[0.78rem] font-semibold tracking-[0.1em] text-ink-70 uppercase',
    control: 'px-[18px] py-4 text-[0.97rem]',
    caretY: '24px',
    checkboxWrapper: 'my-1.5 mb-[26px] flex items-start gap-3 max-sm:gap-3.5',
    checkboxInput: 'mt-px size-[22px] shrink-0 cursor-pointer accent-sage-deep max-sm:size-[26px]',
    checkboxLabel: 'cursor-pointer text-[0.88rem] leading-[1.6] text-ink-70',
  },
  compact: {
    wrapper: 'mb-4',
    label: 'mb-2 block text-[0.86rem] font-medium text-ink',
    control: 'px-[18px] py-[15px] text-[0.95rem]',
    caretY: '22px',
    checkboxWrapper: 'my-1 mb-5 flex items-start gap-3',
    checkboxInput: 'mt-[3px] size-[20px] shrink-0 cursor-pointer accent-sage-deep',
    checkboxLabel: 'cursor-pointer text-[0.85rem] leading-[1.6] text-ink-70',
  },
} as const satisfies Record<FormFieldVariant, Record<string, string>>;

/** The dropdown chevron, drawn with two gradients so it needs no asset. */
