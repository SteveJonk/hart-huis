...
export function MediaBeheer() {
  ...
  const zichtbaar = useMemo(
    () => (assets ?? []).filter((a) => matchesFilter(a, filter) && matchesSearch(a, zoek)),
    [assets, filter, zoek],
  )
  ...
  const ongebruikt = (assets ?? []).filter((a) => !a.inGebruik).length
    ...
    <div style={m.wrapper}>
      ...
      <div style={m.toolbar}>
        ...
        >
          ...
          <option value="ongebruikt">Alleen ongebruikt</option>
        </select>
        <span style={styles.intro}>
          {assets === null
            ? 'Laden…'
            : `${zichtbaar.length} van ${totaal} — ${ongebruikt} ongebruikt`}
        </span>
      </div>
  ...
  return (
    ...
    >
      ...
      <span style={m.cardBody}>
        ...
        {!asset.inGebruik && <span style={m.badge}>ongebruikt</span>}
      </span>
    </button>
  )
}
